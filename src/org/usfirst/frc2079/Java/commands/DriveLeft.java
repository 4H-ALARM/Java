...
public class DriveLeft extends Command {
	...
	protected void execute() {
		if(!RobotMap.autoSwitch.get()) {
			...
			if (DriverStation.getInstance().getGameSpecificMessage().charAt(0) == 'L' && System.currentTimeMillis() > (start + tForwards) && (System.currentTimeMillis() < (start + (tForwards+tTurn)))) {
				RobotMap.dtSCG1.set(0.79); //Turn to the right
				RobotMap.dtSCG2.set(0.80);
			}
				
			if (DriverStation.getInstance().getGameSpecificMessage().charAt(0) == 'L' && System.currentTimeMillis() > (start + (tForwards+tTurn)+2000) && System.currentTimeMillis() < (start + endTime+3000)) {
